...
namespace WebStoreMVC.Services.InMemory
{
    public class InMemoryBlogsService : IBlogsService
    {
        ...
        public async Task<Blog?> GetByIdAsync(int id, CancellationToken cansel = default) => await Task
            .FromResult(TestData.Blogs
            .FirstOrDefault(b => b.Id == id)).ConfigureAwait(false);
    }
}

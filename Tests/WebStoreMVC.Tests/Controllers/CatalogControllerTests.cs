...
public class CatalogControllerTests
{
	...
	public async Task Details_Returns_with_View()
	{
		...
		product_data_mock.Setup(s => s.GetProductById(It.IsAny<int>(), It.IsAny<CancellationToken>()))
		...
		Assert.Equal(expected_brand_name, model.Brand?.Name);
		Assert.Equal(expected_section_name, model.Section!.Name);

		product_data_mock.Verify(s => s.GetProductById(It.Is<int>(id => id == expected_id), It.IsAny<CancellationToken>()));
